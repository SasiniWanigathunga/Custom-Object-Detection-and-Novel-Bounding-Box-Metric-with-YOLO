...
class Custom_Loss(nn.Module):
    ...
    def forward(self, iou, prev_loss, center_loss, aspect_loss):
        term1 = prev_loss 
        term2 = torch.exp(-self.beta * iou) * (center_loss + aspect_loss)
        return term1 + term2
...
class ComputeLoss:
    ...
    def __call__(self, p, targets):  # predictions, targets
        ...
        # Will accumulate additional losses for center and aspect ratio
        total_center_loss = torch.zeros(1, device=self.device)
        total_aspect_loss = torch.zeros(1, device=self.device)
        total_iou_sum = 0.0
        total_targets = 0
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n := b.shape[0]:
                ...
                aspect_loss_layer = torch.abs(1 - min(target_ratio.mean(),pred_ratio.mean())/max(target_ratio.mean(),pred_ratio.mean()))
                # Normalize the tensors so they sum to 1
        ...
        avg_iou = total_iou_sum / total_targets if total_targets > 0 else torch.tensor(0.0, device=self.device)
